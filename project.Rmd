...
```
```{r reading_the_data}
kickstart <- read_csv("kickstart_small.csv")
```
...
kickstart %>%
  ...
  filter(
    state == "canceled" |
      state == 'failed' | state == 'live' | state == 'successful' |
...
ggplot(kickstart, mapping = aes(x = pledged)) +
...
100 for only a small number of projects. We can assume that smaller projects 
would have a higher chance of being successful on this platform
...
m_full <- glm(factor(state) ~ goal + backers + pledged,
...
lm_interaction <- lm(factor(state) ~ pledged + goal + pledged:goal, 
...
ggplot(data = kickstart_train_log,
       mapping = aes(
         y = pledged,
         x = goal,
         color = factor(state)
  ...
  geom_point(alpha = 0.3) +
  labs(color = "State", y = "Pledged Amount", x = "Goal",
       title = "Standardized Scatterplot Pledged Amount vs. Goal") +
  theme_bw(base_size = 16)
...
```{r get_p_value}
slope_val%>%
  get_p_value(obs_stat = sample_stat, direction = "two_sided")
```
...
new_project <- tibble(goal = c(100, 9627.803, 27000),
                      backers = c(150, 253.86, 7500),
                      pledged = c(1000, 21480.74, 45000))
