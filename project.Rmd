...
```{r}
 write.csv(kickstart_1, 'kickstart_small.csv')
```
...
```{r indices}
indices <- sample(nrow(kickstart), 2500)
```
...
```{r regression}
lm_interaction <- lm(state ~ pledged+goal+pledged:goal, 
...
ggplot(data = kickstart_train_log,
       mapping = aes(y = pledged, x = goal, color = factor(state))) +
  geom_point(alpha = 0.3) +
  labs(color = "Goal", y = "pledged amount") +
  theme_bw(base_size = 16)
...
slope_val%>%
  ...
  shade_p_value(obs_stat=sample_stat, direction="two_sided", color="red"
                , pink="green")
