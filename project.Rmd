...
```
```{r knn}
train_knn <- knn(kickstart_train, kickstart_test, train_state, k = 35, prob = F, use.all = F)
#train_knn 

mean(train_knn == true_state)
```

```{r knn_best}
values <- numeric(79) 
for(i in 1:79) {
  trains_knn <- knn(kickstart_train, kickstart_test, train_state, k = i, 
...
kickstart_new %>%
  filter(state == "successful") %>% 
  specify(response = pledged) %>% 
  generate(reps = 100, type = "bootstrap") %>% 
  calculate(stat = "mean") %>%
  get_ci(level = .95)
```

Using these calculations, we are 95% confident that the true successful startup's amount pledged by the backers will be in the (0.1187411, 0.1398106) interval. 
...
Using these calculations, we are 95% confident that the true successful startup's number of backers will be in the (0.133755, 0.1494601) interval. 
...
Using these calculations, we are 95% confident that the true successful startup's requested amount will be in the (-0.03102419, -0.03071722) interval. 	
...
```
QUESTION:
- why no r-squared if factored
-response variable --> can it be categorical?
...
once fit full logistic model, can use step full
then see which variables meaningful
```{r}
...
ggplot(m_pledged_aug, mapping = aes(x = .fitted, y = .resid)) +
